		...
		<div class="d-flex flex-column align-items-center bd-highlight flex-grow-1">
			...
			<div class="container">
				<div class="alert alert-info alert-dismissible">
					...
					<strong>Attention!</strong> Vous ne pouvez pas modifier ou supprimer cette demande!
				</div>
			</div>
			<?php
				$_SESSION["erreurRequest"] = false;
				}
				if((ISSET($_SESSION["requestDeleted"]) && $_SESSION["requestDeleted"] == true))
				{
			?>
			<div class="container">
				<div class="alert alert-info alert-dismissible">
					<a href="#" class="close" data-dismiss="alert" aria-label="close">&times;</a>
					<strong>Succès!</strong> La demande à été supprimé!
				</div>
			</div>
			<?php
				...
				<div class="card">
					...
					<div id="collapse<?=$request->getIdRequest()?>" class="collapse <?=$collapsedShow?>" aria-labelledby="heading<?=$request->getIdRequest()?>" data-parent="#accordionRequest">
						...
						<div class="card-footer text-right">
							<a href='?action=editRequest&idRequest=<?=$request->getIdRequest()?>' title='Éditer la demande'><i class="far fa-edit"></i></a>
