...
class MemberSkillsDAO
{
	...
	public static function findSkills($id)
	{
		 $db = Database::getInstance();
          $favs = Array();
            try {			
                $pstmt = $db->prepare("SELECT m1.idRegistre, m1.idMember, s1.idSkills, s1.description FROM memberskills m1 INNER JOIN skills s1 on m1.idSkill = s1.idSkills and m1.idMember=:id");
